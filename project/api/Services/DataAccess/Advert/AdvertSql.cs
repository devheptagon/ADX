...
class AdvertSqlStrings
{
    ...
    public static string SelectFilterSql =
		...
		WHERE 
			...
			(@TAGS IS NULL OR (SELECT COUNT(0) FROM AdvertTag AS ATA INNER JOIN Tag AS T ON T.id = ATA.tag_id AND ATA.advert_id = ADV.id AND T.title IN (@TAGS)) > 0) AND
			(@TENURES IS NULL OR (SELECT COUNT(0) FROM AdvertTenure AS ATE INNER JOIN Tenure AS TE ON TE.id = ATE.tenure_id AND ATE.advert_id = ADV.id AND TE.title IN (@TENURES)) > 0)
