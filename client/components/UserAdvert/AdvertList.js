...
const AdvertList = ({className, onEdit}) => {
  const dispatch = useDispatch();
  const {data} = useSelector(state => state.adverts);
  ...
  const onEditHandler = (id, title) => {
    const response = confirm(`Do you want to edit ${title} titled advert?`);
    if(response){
      onEdit(id);
      dispatch(selectAdvert(id));
      console.log(id + ' advert edited');
    }
  };
  ...
  const onDeleteHandler = id => {
    const response = confirm(`Do you want to delete ${id} numbered advert`);
    if(response){
      dispatch(deleteAdvert(id));
      dispatch(searchBySectorLocation({page: page, per_page: per_page}));
      console.log(id + ' advert deleted');
    }
  };
      ...
      <Table className="d-flex flex-column" striped bordered hover>
        ...
        <tbody>
          {data?.data?.map((item, index)=>
            <tr key={item.advertID.toString() + index}>
              <td>{firstIndex + index + 1}</td>
              <td>{item.title}</td>
              <td>{item.advertStatus}</td>
